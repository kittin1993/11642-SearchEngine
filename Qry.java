...
public abstract class Qry {
     ...
     * arguments. The SCORE query operator has 1 argument. All other query
     * operators have 1 or more arguments (e.g., #SYN (blue aqua)). Some query
     * operators have parameters (e.g., #NEAR/8) or weights (e.g., #WSUM (0.4
     * blue 0.6 skies) that are not considered arguments.
     */
    protected ArrayList<Qry> args = new ArrayList<Qry>();

    /**
     * The string to use when the query is displayed. Some query operators
     * (e.g., QrySopAnd) may be represented by more than one name (e.g.,
     * #COMBINE, #AND).
     */
    private String displayName = new String("Unnamed");

    /**
     * docIteratorHasMatch caches the matching docid so that docIteratorGetMatch
     * and getScore don't have to recompute it.
     */
     ...
     * 
     * @param q
     *            The query argument (query operator) to append.
     * @throws IllegalArgumentException
     *             q is an invalid argument
     */
    public void appendArg(Qry q) throws IllegalArgumentException {
        ...
        throw new IllegalArgumentException(
                "Objects of type " + q.getClass().getName()
    ...
    /**
     * Advance the internal document iterator to the specified document, or
     * beyond if it doesn't.
     * 
     * @param docid
     *            An internal document id.
     */
     ...
     * docIteratorHasMatch to determine whether the iterator currently points to
     * a document. If the iterator doesn't point to a document, an invalid
     ...
     * document that matches all query arguments; some subclasses may choose to
    ...
    /**
     * An instantiation of docIteratorHasMatch that is true if the query has a
     * document that matches the first query argument; some subclasses may
    ...
    /**
     * An instantiation of docIteratorHasMatch that is true if the query has a
     * document that matches at least one query argument; the match is the
     * smallest docid to match; some subclasses may choose to use this
     * implementation.
     * 
     * @param r
     *            The retrieval model that determines what is a match
    ...
    protected boolean docIteratorHasMatchMin(RetrievalModel r) {
        ...
        // traverse all terms, if there is one, return ture and
        // set the matchcache to the smallest one
        // from the current document that pointer points to 
    ...
    /**
     * Return the cache if there is a match.
     * 
     * @return docId if there is a match or throw an exception.
     */
    protected int docIteratorGetMatchCache() {
        if (this.docIteratorHasMatchCache()) {
            return this.docIteratorMatchCache;
        } else {
            throw new IllegalStateException("No matching docid was cached.");
     ...
     * 
     * @param docid
     *            The internal document id to store in the cache.
     */
    ...
    /**
     * Get the i'th query argument. The main value of this method is that it
     * casts the argument to the correct type.
     * 
     * @param i
     *            The index of the query argument to return.
    ...
    /**
     * Every operator has a display name that can be used by toString for
     ...
     * 
     * @param name
     *            The query operator's display name
     */
    ...
    /**
     * Get a string version of this query operator. This is a generic method
     * that works for most query operators. However, some query operators (e.g.,
     * #NEAR/n or #WEIGHT) may need to override this method with something more
     * specific.
     * 

...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        if (countChars(operatorNameLowerCase, '/') > 0) {
            String[] str = operatorNameLowerCase.split("/");
            operatorDistance = Integer.valueOf(str[1]);
            //System.out.println("distance is " + String.valueOf(operatorDistance));
            operatorNameLowerCase = str[0];
        }
    ...
    private static Qry parseString(String queryString)
        ...
        if ((countChars(queryString, '(') == 0)
                || (countChars(queryString, '(') != countChars(queryString,
                        ')'))
                || (indexOfBalencingParen(
                        queryString) != (queryString.length() - 1))) {
            syntaxError("Missing, unbalanced, or misplaced parentheses");
        }

        /*
         * if (countChars(queryString, '(') != countChars(queryString, ')')) {
         * syntaxError("Missing, unbalanced, or misplaced parentheses"); } if
         * (countChars(queryString, '(') == 0 || countChars(queryString, '(') ==
         * 0 && (indexOfBalencingParen( queryString) != (queryString.length() -
         * 1))) { queryString = "#or(" + queryString + ")"; }
         */
     ...
     */
    static private void syntaxError(String errorString)
            throws IllegalArgumentException {
